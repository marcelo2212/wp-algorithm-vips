    ...
    public function templates_ajax_request(){
        ...
        if ( isset( $_REQUEST ) ) {
            ...
            $args = [
                ...
                'post_status'  => !empty( $page_title ) ? $page_status : 'publish',
            ...
            update_post_meta( $new_post_id, '_elementor_location', 'myCustomLocation' );
            update_post_meta( $new_post_id, '_elementor_conditions', [ 'include/general' ] );
            update_post_meta( $new_post_id, '_wp_page_template', !empty( $page_template ) && !empty( $page_title )  ? $page_template : '' );
    ...
    public function required_theme( $themes, $type ){
        foreach ( $themes as $key => $theme ) {
            ...
            if ( ! is_wp_error( $data ) ) {
                ...
                ?>
                    <li class="etwwptemplate-theme etwwptemplata-theme-<?php echo esc_attr($data['slug']); ?>">
                        <h3><?php echo esc_html($data['name']); ?></h3>
                        <?php
